...
| `auto_run`    | no       | When missing generations start: `"always"` (default, including when viewed), `"settled"` (when the agent transitions out of active work), or `"never"` (no automatic start). Viewing is passive for `"settled"` and `"never"`; Refresh and explicit artifact retry requests still run them. Cached artifacts display in every mode. |
