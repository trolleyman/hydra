...
| 4 | Whitespace-only / indent-only classification + dimming | done | med-high | `wordDiff.ts` `isWhitespaceOnlyChange` + `buildWordRangeMaps.wsOld/wsNew` -> `DiffViewer` dims the row (opacity-50, "Whitespace-only change" title). Note: a block re-indented with a *constant* delta is caught by moved-block detection first (git-consistent), so dimming mostly fires for internal realignments / trailing space / short re-indents |
