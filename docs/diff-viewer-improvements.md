...
| Show whitespace | Built - `web/src/lib/whitespaceMarks.ts` wraps the marked runs of one line's FINAL HTML (after highlighting and the word-diff overlay, so neither has to know) in `.ws-space`/`.ws-tab`, which `index.css` draws a tiled 1ch dot / a full-width arrow on. Off by default; Settings -> Browser -> Whitespace picks off / indent + trailing / all (`lib/whitespacePrefs`). The mark is a background and a pseudo-element AROUND the real character, never a substituted glyph, so copied code is unchanged and nothing moves where a line wraps (which `diffMetrics`'s height probe relies on). Shared with `CodePane`/`DiffPane` and the chat's file cards. |
...
| 7 | Moved-block detection (zebra + allow-indentation-change) | tried, reverted | high | Built as a client-side `--color-moved=zebra` (line-by-line violet/sky tint) then removed - the whole-block colour wash read as heavy/confusing, and an in-place re-indent registering as a "move" was more noise than signal. If revisited, do it **properly** - full design in [diff-moved-blocks.md](diff-moved-blocks.md) (quiet boxes + per-move colour identity + "moved from/to path:line" click-to-jump; no literal cross-card connectors). The removed algorithm (intern by content, greedy constant-indent-delta block growth, 20-alnum threshold) is in git history if useful |
