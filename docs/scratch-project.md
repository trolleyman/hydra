# Scratch project (the built-in "just chatting" project)

A always-present, Hydra-owned project that exists so you can start a conversation
without pointing Hydra at a repo first. It is the landing place on first run, and
the fallback whenever there is nowhere else to land.
...
The scratch project fills that hole: a real project, owned and created by Hydra,
that always exists.
...
- **Stored at `~/.local/share/hydra/scratch`** (`$XDG_DATA_HOME/hydra/scratch`).
...
- **Its project ID is `_scratch`** - collision-proof by construction, see below.
...
- **`/` stops being a dead end**, falling back to `/project/_scratch/` when there
...
- **It is permanently listed** in the project dropdown, pinned above a divider,
  with its path suppressed (`~/.local/share/hydra/scratch` is noise, not
  information). It must be listed: the dropdown button renders the *current*
  project's icon and name, so an unlisted scratch project would leave the top bar
...
- **The Ctrl+` switcher is deliberately left alone.** It is an alt-tab list
  ...
  break the "one tap = previous project" model it exists for. Scratch takes part
  in recency like any other project.
...
- **The sandbox has nowhere to put the agent.** On Linux an empty
  ...
  falls back to whatever cwd `hydrad` happens to have (`internal/sandbox/linux.go`).
...
So: give the scratch project a real repo with a real initial commit (a worktree
...
accident: merging a scratch head into the scratch `main` archives that
conversation's files.

## Why the ID is `_scratch`
...
the generator**, which makes `_scratch` collision-proof by construction rather
than by a reserved-names list.

Plain `scratch` would be actively worse, not merely risky. `generateID` builds
its `existing` set from the current project list, and the built-in is in that
list - so a user registering `~/code/scratch` would be deduped to `scratch2`
while the built-in kept the good name. The user's real project gets the ugly ID.

A leading hyphen is also unreachable, but `-scratch` is miserable to pass as a
...
`selectedProjectId` - the user's *last real project*. Rendering the scratch
composer inline at `/` would therefore show a scratch chat box in the main pane
while the sidebar listed a different project's agents and the dropdown showed a
...
Redirecting makes scratch a project you are genuinely *in*, so the sidebar,
dropdown, drafts and view state all work with no new chrome.

### ...but it is rarely the landing page, by design

It is tempting to describe scratch as "where you land on first run". It mostly
is not, and that is deliberate. Project selection on boot is decided in
...
4. (new) the built-in scratch project.
...
is correct - overriding it to force scratch to the front would be a real
regression, so the scratch fallback deliberately sits at the bottom of the
chain and fires only when there is genuinely nothing else (no remembered
project, no default, no user projects).

The practical entry points to scratch are therefore the project dropdown and the
Ctrl+` switcher, not the URL. What matters is that it always *exists* and is one
...
The zero-state copy is deliberately *kept* rather than deleted. Scratch
...
- **Bootstrap must complete before the server serves.** Every route and endpoint
  ...
  `Server.resolveProjectRoot`, which 404s on a miss). If the scratch project were
  created lazily on first visit, the first navigation to `/project/_scratch/`
...
- **Project IDs are never filesystem paths.** The ID is only ever a lookup key
  ...
  `/project/_scratch/` *should* be visitable, since the dropdown, the Ctrl+`
...
- **Only the branch picker is hidden, not the diff or merge.** It is tempting to
  strip all git chrome from the scratch project, but scratch heads write real
  files, so the diff is genuinely useful ("what did that conversation produce?")
  and merging into the scratch default branch is how you keep a conversation's
  output. What *is* noise is `SpawnForm`'s base-branch picker - there is no work
  to stack on - so that is the only surface gated on `builtin`. Note this cannot
  be done by branch-gating: `DiffViewer` already no-ops when `branch_name` is
  null, but scratch heads do have branches.

## Build order

1. `ProjectInfo.Builtin` + `EnsureScratchProject` (git init, initial commit,
...
- **Hydra control tools** (spawn a head in another project from a scratch chat).
  ...
  worth taking on its own. The scratch repo's own `.hydra/config.toml` is the
  ...
  not inherit it.
- **Cross-project reads.** A scratch head is sandboxed into its own worktree and
