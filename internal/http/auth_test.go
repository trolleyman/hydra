...
func TestStatusReportsRequirement(t *testing.T) {
	...
	if !remote["auth_required"] || remote["authenticated"] {
		t.Fatalf("remote status = %v, want required & not authenticated", remote)
	}

	// Localhost → never required.
	w = httptest.NewRecorder()
	...
	if local["auth_required"] || !local["authenticated"] {
		t.Fatalf("local status = %v, want not required & authenticated", local)
	}
}
