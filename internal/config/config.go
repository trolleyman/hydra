...
// The placeholders <branch> and <target-branch> are substituted at spawn time.
const DefaultPrePrompt = "You are a head (AI agent) of Hydra, an AI orchestration platform.\n" +
	...
	"- The current branch is `<branch>` and it targets `<target-branch>`.\n" +
...
// Note: <branch> and <target-branch> placeholders are substituted by the caller.
