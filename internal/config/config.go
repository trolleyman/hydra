...
func artifactsDocLines() []string {
	return []string{
		...
		docPrefix + " Formats: .png, .jpg and .gif are diffed pixel-by-pixel (cosmetic re-encodes are",
		docPrefix + " ignored). Other supported types — .webp .avif .svg .bmp .pdf and .webm video — are",
		docPrefix + " compared by byte hash, so any non-deterministic encode reads as \"modified\". Video",
		docPrefix + " is supported via .webm ONLY and must be LOSSLESS (e.g. ffmpeg ... -c:v libvpx-vp9",
		docPrefix + " -lossless 1) to keep identical frames identical and avoid spurious diffs.",
