...
func artifactsDocLines() []string {
	return []string{
		...
		docPrefix + " Formats: still images (.png .jpg .gif .webp .avif .svg .bmp) and .pdf are diffed",
		docPrefix + " pixel-by-pixel. Video is supported via .webm ONLY — and it must be LOSSLESS",
		docPrefix + " (e.g. ffmpeg ... -c:v libvpx-vp9 -lossless 1). Video is compared by byte hash, not",
		docPrefix + " pixels, so any lossy/non-deterministic encode always reads as \"modified\" and",
		docPrefix + " yields spurious diffs; a lossless encode keeps identical frames identical.",
