...
// bash-shell tabs). Lives in lib (not AgentTerminal.tsx, its main consumer) so
// the chat pane can share it without the component file exporting a non-component
// (which breaks react-refresh).
import { loadLastGeometry } from './terminalGeometry'

export function getWsUrl(agentId: string, projectId: string | null, shell?: boolean, sandboxed?: boolean, shellId?: string): string {
