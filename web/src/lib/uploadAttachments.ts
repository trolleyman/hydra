...
// An absolute path ending in an upload name generated by uniqueUploadName.
// The state root is configurable and uploads live below its per-project tree,
// so neither the root nor the directories before `uploads` are fixed. Requiring
// the generated nanosecond prefix keeps an unrelated source path containing an
// `uploads` directory from being mistaken for a prompt attachment.
export const UPLOAD_PATH_RE = /(?:\/[^\s/]+)*\/uploads\/\d+-[A-Za-z0-9._-]+/g
