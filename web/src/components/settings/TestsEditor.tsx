                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-gray-400 dark:text-gray-500 flex items-center gap-1">
                        ...
                        <InfoTooltip title="Automatic runs">
                          <p><strong>Always</strong> starts a missing run when tests are viewed. <strong>When agent settles</strong> waits while the agent is actively working. <strong>Never</strong> only runs when you use Refresh.</p>
                          <p className="mt-1.5">Cached verdicts are still shown in every mode, and Refresh always runs immediately.</p>
                        </InfoTooltip>
                      </label>
                      <select
                        value={t.auto_run ?? 'always'}
                        onChange={(e) => update(index, { auto_run: e.target.value === 'always' ? undefined : e.target.value as NonNullable<TestScript['auto_run']> })}
