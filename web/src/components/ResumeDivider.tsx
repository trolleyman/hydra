...
export function ResumeDivider({ resumedAt, ariaLabel = 'Agent resumed' }: { resumedAt?: number; ariaLabel?: string }) {
  const label = (
    <span className="optical-center text-2xs text-stone-400 dark:text-stone-500">
      Resumed{resumedAt == null ? null : <> <RelativeTime createdAt={resumedAt / 1000} /></>}
    </span>
  )
  return (
