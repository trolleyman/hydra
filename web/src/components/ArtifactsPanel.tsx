...
function fileMatchesFilter(file: ArtifactFile, filter: ArtifactTagFilter): boolean {
  ...
  for (const [cat, off] of Object.entries(filter.scoped)) {
    if (off.length > 0 && off.some((v) => tags.includes(`${cat}::${v}`))) return false
  }
