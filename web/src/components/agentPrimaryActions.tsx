...
export function agentPrimaryActionAppearances({
  ...
  const noun = (agent.review?.provider ?? provider) === 'github' ? 'PR' : 'MR'
  ...
  const merge: AgentPrimaryActionAppearance = merging
    ...
    : agent.merge_when_green
      ...
      : { command: 'merge', label: 'Merge', icon: <GitPullRequestArrow className="w-4 h-4" />, disabled: busy }
  ...
  return [
    ...
    { command: 'restart', label: 'Restart', icon: <RotateCcw className="w-4 h-4" />, disabled: merging || killing || restarting },
    { command: 'kill', label: 'Kill', icon: <Trash2 className="w-4 h-4" />, disabled: merging || killing, danger: true },
  ]
}
