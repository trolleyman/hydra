 ...
 */
export function FilePathLabel({ path, className = '', trailing, nativeTitle = true, wrap = false }: {
  path: string
  className?: string
  trailing?: ReactNode
  /** Suppress the browser tooltip when this label already sits in <Tooltip>. */
  nativeTitle?: boolean
  /** Wrap the complete path when the label is itself revealing truncated text. */
  wrap?: boolean
}) {
  ...
  return (
    <span className={`inline-flex min-w-0 items-center gap-1.5 ${className}`} title={nativeTitle ? path : undefined}>
      ...
      <span data-tooltip-overflow className={`min-w-0 ${wrap ? 'whitespace-normal break-words' : 'truncate'}`}>
