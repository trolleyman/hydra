...
34. [x] **[Diff viewer]** **Selecting text in the diff occasionally resets the selection.** A background "silent" refresh (fired when a git command is detected in the agent terminal — `externalRefreshTrigger` in `DiffViewer.tsx`) calls `setDiff(d)` with a brand-new `DiffResponse`, so every `file` object gets a new reference and every memoized `FileDiff` re-renders, replacing the DOM nodes the browser's text selection was anchored to. The selection collapses mid-drag. _Fixed:_ the silent-refresh path now defers applying the fetched diff while the user has a non-collapsed text selection inside the diff viewer. The pending diff (plus its captured per-file context snapshot) is held in a ref and flushed by a `selectionchange` listener once the selection clears; if a newer silent refresh arrives first it simply replaces the pending one (latest wins). User-initiated refreshes (manual refresh button, commit-selector changes) are unaffected — losing the selection there is expected.
