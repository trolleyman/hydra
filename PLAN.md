...
26. [ ] **[Security]** **Run artifact generator commands inside the sandbox.** Today `internal/artifacts/artifacts.go` runs each `[[artifacts]]` command directly on the host (`exec.CommandContext("sh", "-c", spec.Command)` with `os.Environ()`) — no bwrap/sandbox-exec, unlike agents and the web shells. This is a documented #17 follow-up, but it is a real **sandbox-escape / RCE**, not just "trusted config runs on the host":
