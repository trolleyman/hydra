...
35. [ ] **[Diff viewer]** **Stop emitting the unconditional 20s `diff_refresh` ticker; only refresh when the worktree actually changed.** The terminal WS goroutine (`internal/http/terminal.go`) sends `diff_refresh` on a 20-second timer regardless of whether anything changed, forcing the frontend to re-fetch the full diff every 20s per attached client. #34 made the frontend ignore no-op results (diff the response, skip if identical), so this is now wasted work rather than a correctness bug — but it still costs a full `getAgentDiff` round-trip on every idle tick. _Follow-up:_ drive `diff_refresh` from real change signals instead — e.g. an fs-watcher on the worktree (debounced) and/or a content hash of the diff computed server-side — and drop or greatly lengthen the blind timer. Keep the git-command detection path as a fast trigger.
