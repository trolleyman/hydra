import { ChevronRight, ExternalLink, GitBranch, Network, Plus, Save, Server, Settings2, Shield, Trash2, Wrench } from 'lucide-react'
import React, { useEffect, useState, type ReactNode } from 'react'
import { postMessage } from '../bridge'
import type { ViewState } from '../types'
import { Button, Field, InfoTip, PageHeading } from './ui'
  ...
  return <section className="min-h-0 flex-1 overflow-y-auto px-3 py-4">
    <div className="mx-auto max-w-3xl">
      ...
      <div className="flex flex-col gap-2.5">
        <SettingsSection icon={<Settings2 />} title="Identity" explanation="The standing prompt is loaded as session instructions when the provider starts. It is not appended to every user turn." open>
          ...
          <Field label="Standing prompt"><textarea rows={5} value={draft.prompt ?? ''} onChange={event => set(['prompt'], event.target.value)} /></Field>
        </SettingsSection>
        <SettingsSection icon={<Wrench />} title="Core tools" explanation="Allow makes a tool available without a tool-level prompt; Ask requests approval; Deny removes it. These decisions never bypass the sandbox: Read and Search still require readable paths, Edit requires writable paths, and Bash and Fetch still obey filesystem and network policy." open>
        ...
        <SettingsSection icon={<Shield />} title="Filesystem" explanation="Relative paths resolve from the workspace. Readable paths permit inspection, writable paths permit mutation, copy-on-write paths isolate changes, and masked paths stay hidden even if another rule permits access.">
        ...
        <SettingsSection icon={<Network />} title="Network" explanation="Network policy applies to every sandboxed process, including commands started by Bash. Hard mode holds unknown hosts for approval; blocked hosts remain unavailable.">
        ...
        <SettingsSection icon={<GitBranch />} title="Git" explanation="Git operations have their own decisions and protected-branch checks. An allowed Git operation still cannot write outside the filesystem sandbox.">
...
function SettingsSection({ icon, title, explanation, open = false, children }: { icon: ReactNode; title: string; explanation: string; open?: boolean; children: ReactNode }) {
  return <details open={open} className="settings-section group"><summary className="flex cursor-pointer list-none items-center gap-2.5 px-3 py-2.5"><span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-[var(--hydra-accent-soft)] text-[var(--vscode-textLink-foreground)] [&>svg]:size-3.5">{icon}</span><strong className="min-w-0 flex-1 text-xs font-semibold">{title}</strong><InfoTip>{explanation}</InfoTip><ChevronRight className="size-3.5 text-[var(--vscode-descriptionForeground)] transition-transform group-open:rotate-90" /></summary><div className="grid gap-3 border-t border-[var(--hydra-border-subtle)] px-3 py-3">{children}</div></details>
}

function DecisionRow({ label, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) {
  return <label className="flex min-h-8 items-center justify-between gap-3 border-b border-[var(--hydra-border-subtle)] py-1.5 last:border-b-0"><span className="text-xs">{label}</span><select className={`w-32 shrink-0 decision-${value}`} value={value} onChange={event => onChange(event.target.value)}>{values.map(option => <option key={option} value={option}>{sentenceCase(option)}</option>)}</select></label>
}
...
function sentenceCase(value: string): string { return value[0].toUpperCase() + value.slice(1).replaceAll('_', ' ') }
