...
Status: all five sections implemented 2026-07-07 (see the checked items below).
...
- [x] `.hydra/config.toml`: demo command binds all interfaces -
      `HYDRA_API_ADDR=0.0.0.0:$HYDRA_PREVIEW_PORT` (safe: private netns).
- [x] Probe with a real HTTP GET (any response counts as ready) instead of a bare
      dial, so pasta's accept can't false-positive.
- [x] When the dial succeeds but requests keep getting refused under hard mode,
      append a log hint: "server must bind 0.0.0.0 under network mode hard".
- [x] Export `HYDRA_PREVIEW_ADDR` (host included, mode-aware: `0.0.0.0:NNNN`
...
- [x] "Start the preview server without opening a tab" -> "Start the server".
- [x] Use the lucide `Play` icon instead of the circular arrow.
- [x] Drop the "Build log" header above the log output.
...
- [x] Introduce a stable "slot" per script that owns the proxy port; keyed
      instances become an implementation detail behind it.
- [x] Selection -> channel mapping: pinned commit -> that commit's checkout;
      "Latest commit" -> branch-tip channel (see 4); "Latest changes" ->
      worktree channel (see 5).
- [x] Remove the `Others` listing from the panel.
...
- [x] When the tip moves, spawn the new SHA's instance in the background
...
- [x] No config knob for now; revisit an `[[artifacts]]` option (checkout-in-
...
- [x] Run the server in its own checkout of the worktree's current state.
- [x] While (and only while) the server is running, poll `git status
      ...
      does not).
- [x] File sync only visibly updates HMR-style servers; for build-then-serve,
